...
def show_results_video(result_queue,
    ...
    if len(result_queue) != 0:
        ...
        selected_label, score = results[0]
        location = (0, 40)
        text = selected_label + ': ' + str(round(score, 2))
        text_info[location] = text
        for frame in batch_frames:
...
def show_results(model, data, label, args):
    ...
    result_queue = deque(maxlen=args.batch_size)
    ...
    while ind < num_frames:
        ...
        if ind % args.frame_skip == 0:
            # batch_frames.append(np.array(frame)[:, :, ::-1])
            batch_frames.append(np.array(frame)[..., ::-1])
            idx_list.append(ind)
        
        if len(batch_frames) == args.batch_size or ind == num_frames:
            if len(batch_frames) == args.batch_size:
                print("Processing frames from {} to {}".format(idx_list[0], idx_list[-1]))
                ret, scores = inference(model, data, args, batch_frames)
                ...
                if args.out_file.endswith('.json'):
                    ...
                    text_info = show_results_video(result_queue, text_info,
                                                args.threshold, msg, batch_frames,
...
def inference(model, data, args, frame_queue):
    ...
    cur_data.update(
        dict(
            array=cur_windows,
            modality='RGB',
            frame_inds=np.arange(len(cur_windows))))
