...
class MViTHead(BaseHead):
    ...
    def __init__(self,
                 num_classes: int,
                 in_channels: int,
                 loss_cls: ConfigType = dict(type='CrossEntropyLoss', class_weight=torch.tensor([1.0967, 11.0714, 2.7035, 0.8424, 0.58125, 0.58125])),
        ...
        # self.linear = nn.Linear(self.num_classes, 6)
    ...
    def forward(self, x: Tuple[List[Tensor]], **kwargs) -> Tensor:
        ...
        # [N, num_classes]
        # cls_score = self.linear(cls_score)
        return cls_score
