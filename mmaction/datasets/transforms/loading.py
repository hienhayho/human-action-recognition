...
class SampleFrames(BaseTransform):
    ...
    def _get_test_clips(self, num_frames: int,
        ...
        # clip_offsets = np.arange(num_frames)
        # print(clip_offsets)
        # input()
        return clip_offsets
    ...
    def transform(self, results: dict) -> dict:
        ...
        # print("ori_clip_len", ori_clip_len)
...
# @TRANSFORMS.register_module()
# class ModiFySample(BaseTransform):
...
class DenseSampleFrames(SampleFrames):
    """Select frames from the video by F.
    ...
    def _get_test_clips(self, num_frames: int) -> np.array:
        ...
        clip_offsets = list()
        for start_idx in start_list:
            clip_offsets.extend(base_offsets + start_idx)
        clip_offsets = np.array(clip_offsets)
        # print(clip_offsets)
        # input()
        return clip_offsets
